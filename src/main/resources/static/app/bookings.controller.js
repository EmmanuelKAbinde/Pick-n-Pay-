...
var driverID;
		...
		$http({
			...
			for (var x = 0; x < $scope.Driver.length; x++  )
				...
				{
				if ($scope.Driver[x].username == $scope.UserInfo[0].email )
		...
		var DeliveryInfor = 
		{
			...
			"driverId" : $scope.DeliveryForm.driverId,
				  ...
				  $http({
					...
					}).then(function successCallback(response) 
					{
						...
						for (var x = 0; x < $scope.Driver.length; x++ )
						  ...
						  if ($scope.Driver[x].driverId == driverID)
							  {
							  
							var updateDriver =  {
								 
									 "driverId" : $scope.Driver[x].driverId,
									 "driverSurname" : $scope.Driver[x].driverSurname,
									 "driverName" : $scope.Driver[x].driverName,
						...
						$http({
									method : 'PUT',
									url : '/UpdateDriver/' + driverID,
									data : angular.toJson(updateDriver),
									headers : {
										'Content-Type' : 'application/json'
									}
								})
 ...
 */
	...
	$scope.AddDriver = function(code) {
		...
		var DriverInfor = 
		{
			...
			"username" : $scope.UserInfo[0].email,
